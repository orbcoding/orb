source "$_orb_root/scripts/initialize_variables.sh"

source "$_orb_root/functions/utils/pass.sh"
source "$_orb_root/functions/utils/argument.sh"
source "$_orb_root/functions/utils/debug.sh"
source "$_orb_root/functions/utils/error.sh"
source "$_orb_root/functions/utils/file.sh"
source "$_orb_root/functions/utils/text.sh"
source "$_orb_root/functions/utils/utils.sh"

source "$_orb_root/functions/call/extensions.sh"
source "$_orb_root/functions/call/settings.sh"
source "$_orb_root/functions/call/namespace_and_function.sh"

source "$_orb_root/functions/declaration/arguments.sh"
source "$_orb_root/functions/declaration/argument_options.sh"
source "$_orb_root/functions/declaration/checkers.sh"
source "$_orb_root/functions/declaration/getters.sh"
source "$_orb_root/functions/declaration/function.sh"
source "$_orb_root/functions/declaration/orb_settings_declaration.sh"
source "$_orb_root/functions/declaration/validation.sh"

source "$_orb_root/functions/arguments/assignment.sh"
source "$_orb_root/functions/arguments/checkers.sh"
source "$_orb_root/functions/arguments/collection.sh"
source "$_orb_root/functions/arguments/defaults.sh"
source "$_orb_root/functions/arguments/positional.sh"
source "$_orb_root/functions/arguments/validation.sh"

source "$_orb_root/functions/call/help.sh"

source "$_orb_root/scripts/trace.sh"
