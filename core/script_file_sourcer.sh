...
script_dir="$_orb_dir/scripts/$_script_name"
[[ -f "$script_dir/_${_script_name}.sh" ]] && source "$script_dir/_${_script_name}.sh"
...
if [[ -n $_orb_extensions && -f $_orb_extensions/${_script_name}.sh ]]; then
 	_current_script_extension="$(realpath --relative-to "$script_dir" "$_orb_extensions/${_script_name}.sh")"
