_orb_set_function_arg_default_values() {
  local _orb_default _orb_default_presence
  ...
  local _orb_arg; for _orb_arg in "${_orb_declared_args[@]}"; do
    if ! _orb_has_arg_value $_orb_arg; then
      ...
      elif _orb_get_arg_option_value $_orb_arg "DefaultPresence:" _orb_default_presence; then
        orb_first_present _orb_default "$_orb_default_presence" && \
        _orb_assign_arg_value $_orb_arg "${_orb_default[@]}"
